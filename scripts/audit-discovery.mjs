...
try {
  ...
  assert.equal(await page.getByRole('link', { name: 'Explore Jet Set LatAM on the App Store →' }).getAttribute('href'), 'https://apps.apple.com/us/app/jet-set-latam/id6810912801')
  await page.getByRole('link', { name: 'Find a pairing →', exact: true }).click()
  ...
  assert.equal(await page.getByRole('link', { name: 'Explore Jet Set LatAM on the App Store →' }).count(), 0)
